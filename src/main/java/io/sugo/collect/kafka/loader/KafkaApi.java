...
public class KafkaApi implements Closeable {
  ...
  public void subscribe() {
    List<PartitionInfo> partitions = consumer.partitionsFor("wuxianjiRT");
    for (PartitionInfo info : partitions) {
      ...
      for (TopicPartition topicPartition : topicPartitions) {
        ...
        log.info(String.format("Partition{topic:%s, partition:%d, start offset:%,d, end offset:%,d}",
  ...
  public void printTopic() {
    ...
    for (Map.Entry<String, List<PartitionInfo>> entry : topicMap.entrySet()) {
      log.info(String.format("topic:%s, partitions:%d", entry.getKey(), entry.getValue().size()));
      for (PartitionInfo info : entry.getValue()) {
        log.info(String.format("PartitionInfo{partition:%d, inSyncReplicas:%s, leader:%s, replicas:%s}",
