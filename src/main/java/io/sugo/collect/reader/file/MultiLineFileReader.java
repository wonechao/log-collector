 ...
 */
public class MultiLineFileReader extends AbstractReader {
  private static final String UTF8 = "UTF-8";
  private final Logger logger = LoggerFactory.getLogger(MultiLineFileReader.class);
  ...
  public static final String MULTI_LINE_PATTERN = "multiline.pattern";
  ...
  public MultiLineFileReader(Configure conf, WriterFactory writerFactory) {
          ...
          do {
            ...
            if (tempString == null) {
              ...
              if (messages.size() > 0) {
                ...
                if (multiLineSB.length() != 0) {
                  if (parser == null) {
                    messages.add(multiLineSB.toString());
                  } else {
                    try {
                      Map<String, Object> gmMap = parser.parse(multiLineSB.toString());
                  ...
                  currentByteOffset += multiLineSize;
                  multiLineSB.delete(0, multiLineSB.length());
                }
            ...
            if (StringUtils.isNotBlank(tempString) && tmpSize < maxSize) {
              ...
              // 首行匹配
              if (Pattern.matches(conf.getProperty(MULTI_LINE_PATTERN), tempString)) {
                if (multiLineSB.length() != 0) {
                  if (parser == null) {
                    messages.add(multiLineSB.toString());
                  } else {
                ...
                multiLineSB.append(tempString);
                multiLineSize += (tmpSize + 1);
                line++;
              } else {
                multiLineSB.append(tempString);
                multiLineSize += (tmpSize + 1);
